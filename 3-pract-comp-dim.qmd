---
title: "Assignment: Comparing dimensions"
execute: 
  warning: false
  message: false
format:
...
We will rely on census data for the province of Zaragoza (Spain) in 1860. The data set we will work with contains the individual records for all the population living in the 70 municipalities within 40 kilometers from the capital city of Zaragoza (see map below).^[There are 78 locations because a few municipalities are composed of different settlements.] You can download the data <a href="data-assign/zgz-1860/sample-zgz-1860.csv" download>here</a> [@beltran2024].
...
As always you need to open a new script and set the necessary steps to clear the environment, set the working directory and import the data. I have the  file, named `sample-zgz-1860.csv`, in the folder `data-assing`, so the code below reads it into R.^[The function `read_csv()` forms part of the tidyverse, so it becomes available when you load the `tidyverse`.] 
...
The symbol `<-` (called assignment operator) takes the object that `read_csv()` imports and stores it in the environment with the name you indicate (`data` in this case). You can then type the name of the object and have a sense of how the data looks like:
...
The results show that 24.1 per cent of the population was able to read and write.^[Doing the same with the variable *read* would compute the numbers of those who were able to read but not write.] We are now going to compute literacy rates for males and females separately. We could do it using `count()` but we can also explore how to do it creating a dummy variable that assigns the values 1 or 0 depending on whether the variable *write* equals "SI" (yes) or not. We do that and store the results by over-writing the object `data`. It is always advisable to check what we have done. To do so, we select the columns containing the info we are interested in (we add also *name* and *sex* to provide a bit of context).
...
The patterns in the graph above also changed around 1845 and literacy rates declined significantly until reaching 0 in 1860. This basically reflects the fact those born in those years are children and therefore have not started or concluded their education. To better see how literacy rates change as boys and girls grew up we now plot literacy by age and restrict the analysis to those aged 4-14: 
...
Another potential explanation is that less literate people are also less numerate and therefore have more difficulty remembering their actual age. It is possible to test whether this is actually the case. We replicate part of the code above to classify individuals into groups depending on whether they age-heap (*heap_0*). It is now straightforward to compute the average literacy rate for the individuals in each of those categories using `group_by()` and `summarise()`. We focus on individuals aged 25-45 to have a more homogeneous sample and make the computations for men and women separately.^[Feel free to choose a different age-group but make sure that you at least have 10 age categories, so all last digits (0-9) are possible.] 
...
Let's finish here but, as mentioned above, the topics of study that these sources allow studying are quite varied. Let me also stressed that this exercise was intended not only as a way of solidifying what you have learned so far but also to show that, when explored creatively, the sources contain way more information than it looks like at first sight. If you want more examples you can check the article [What was in a name? Culture, naming practices and education in the past](https://fjbeltrantapia.github.io/papers/names-education-2024.pdf), which, relying on an extended version of the same dataset we have explored here, shows that girls bearing more common names had lower literacy rates. The article argues that first names provide historians with information about the cultural values of the parents bestowing those names. As well as enabling classifying individuals according to their names (after religious figures, after family members, after kings or other political figures, etc.), this approach allows testing whether those values translated into the way that parents raised their children (i.e. the value they attach to education, etc.).  
-->
