...
The [National Biblioteket](https://www.nb.no/search) has million of digitised resources (books, manuscripts, newspapers, letters, etc.). As well as hosting an online [norsk n-gram](https://www.nb.no/ngram/#1_1_1__1_1_3_1810%2C2022_2_2_2_12_2) for quering the corpus, the [DH-LAB](https://www.nb.no/dh-lab/digital-tekstanalyse/) is also creating its own tools for accessing and analysing their collections, including [dhlabR](https://github.com/NationalLibraryOfNorway/dhlabR) an R package to directly interact with the corpus. It is still in development but some features work and allows accessing its massive corpus: you can have a look on their [collections](https://www.nb.no/search?mediatype=brev-og-manuskripter)

![National Biblioteket digital collection.](images/nb.png){width=100% height=100% fig-align="center"}

The package `dhlabR` interacts with the API of National Biblioteket. Although it does not allow to download full texts, you can query the collection, get the necessary metadata (including text IDs) and extracting text-derived outputs such as token frequencies, text chunks, concordances, collocations, and the like. This process therefore comprises two steps:
...
2. Request derived text data: term frequencies, text chunks, concordances, collocations, etc. Instead of extracting full texts directly, you obtain representations tied to those document IDs, usually as a data frame or list that you can use in R for analysis and visualisation. 
...
It is useful to call `glimpse()` to have an overview of all the metadata that you are retrieving.

```{r}
corpus |> glimpse()
```

You can use this metadata to better define your corpus of interest or your subsequent analysis.
...
The example above retrieves a corpus of books ("digbok": novels, monographs, plays, etc.), but defining the argument `doctype` differently allows accessing other materials, such as newspapers ("digavis"), journals and periodicals ("digitidsskrift"), letters and manuscripts ("digimanus") or images ("digifoto"). The example below retrieves 5 newspapers published between 1904 and 1905. Note that I am setting `lang = NULL` to avoid the defaul language ("nob"; bokmål, the official Norwegian language), which may make less sense in historical documents. The metadata contains potentially interesting information such as "title", place of publication ("city") or date of publication ("year" or "timestamp"). 

```{r}
get_document_corpus(
  doctype = "digavis", 
  from_year = 1904, to_year = 1905,
  lang = NULL,
  limit = 5)
```
...
See more details [here](https://github.com/NationalLibraryOfNorway/dhlabR/tree/main/R).

<!-- NOT POSSIBLE

# Getting full texts 

Depending on copyright restrictions, it is sometimes possible to retrieve the full OCR text of newspaper pages, but usually after identifying the documents/pages you are interested in. Note that I am setting `lang = NULL` to avoid the defaul language ("nob"; bokmål, the official Norwegian language), which may make less sense in historical documents.

```{r}
news_1905 <- get_document_corpus(
  doctype = "digavis",
  from_year = 1904,
  to_year = 1905,
  limit = 5,
...
We could use `map_chr()` and `map_int()` as above to extract the values of the columns. However, let's move on directly into our aim here: obtaining the full texts. This is done with the function `get_document_text()`.


Once you have the document identifiers, you request the OCR text.
A typical function is:

What happens here:
corpus_1905$docid contains the document IDs returned by the corpus query.
get_document_text() sends those IDs to the DH-LAB API.
The API returns the OCR text associated with each document.
The result is usually a data frame with one row per document, containing something like
